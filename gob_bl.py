bl_info = {
    "name": "GoB SP Bridge",
    "author": "Cloud Guy | links.cloud3dworks.art",
    "version": (1, 0, 0),
    "blender": (4, 5, 0),
...
def documents_bridge_root():
    ...
    if sys.platform == "darwin":
        ...
        if local_docs.is_dir():
            return local_docs / "GoB_SP_Bridge"
        icloud_docs = (
            Path.home()
            / "Library"
            / "Mobile Documents"
...
def _has_any_suffix(name, *suffix_sets):
    return any(
        suffixes and is_name_with_suffix(name, suffixes) for suffixes in suffix_sets
    )
...
def collect_low_poly_objects(context, prefs):
    ...
    high_suffixes = parse_suffixes(getattr(prefs, "high_poly_suffixes", ""))
    cage_suffixes = parse_suffixes(getattr(prefs, "cage_poly_suffixes", ""))
    ...
    if get_identify_mode(scene) == "COLLECTIONS":
        ...
        if not collection_in_scene(scene, low_collection):
            low_collection = None
        if low_collection:
            for obj in collect_collection_meshes(
                low_collection,
                selected_only=selected_only,
                selected_names=selected_names,
            ):
                if _has_any_suffix(obj.name, high_suffixes, cage_suffixes):
                    continue
...
def write_active_blender_info(context=None, prefs=None):
    ...
    for path in active_blender_info_paths(prefs, project_dir):
        try:
            ...
            continue
    _bridge_conflict_info = conflict
...
UDIM_TILE_RE = re.compile(r"[._](10\d\d|1\d\d\d)(?=[._]|$)")


def split_udim_tile(stem):
    match = UDIM_TILE_RE.search(stem)
    if not match:
        return stem, None
    stripped = stem[:match.start()] + stem[match.end():]
...
def group_textures(texture_paths):
    ...
    for path in texture_paths:
        path_obj = Path(path)
        stem = path_obj.stem
        stem, udim_tile = split_udim_tile(stem)
        ...
        if udim_tile is not None:
            tile_num = int(udim_tile)
            tile_key = (texset, map_type)
            if tile_key not in udim_tiles or tile_num < udim_tiles[tile_key]:
                udim_tiles[tile_key] = tile_num
                grouped.setdefault(texset, {})[map_type] = {"path": path, "udim": True}
        else:
            grouped.setdefault(texset, {})[map_type] = path
    return grouped


def _map_path_and_udim(value):
    if isinstance(value, dict):
        return value.get("path"), bool(value.get("udim"))
    return value, False


def load_image(path, udim=False):
    try:
...
def build_material(mat, maps, normal_y_invert=False, manifest=None):
    ...
    roblox = bool(manifest and manifest.get("roblox"))
    ...
    for map_type in ("base_color", "orm", "metallic_roughness", "metallic_smoothness",
        ...
        if map_type not in maps:
            continue
        if roblox and map_type in ("specular", "specular_smoothness"):
            continue
        tex = nodes.new("ShaderNodeTexImage")
        tex.location = (-400, y)
        y += step
        map_path, map_udim = _map_path_and_udim(maps[map_type])
        image = load_image(map_path, udim=map_udim)
        if not image:
            continue
        tex.image = image
    ...
    if base_node and ao_output and not roblox:
    ...
    if normal_node:
        ...
        if normal_y_invert and not roblox:
            separate = nodes.new("ShaderNodeSeparateColor")
    ...
    if opacity_output:
        ...
        if hasattr(mat, "blend_method"):
            mat.blend_method = "BLEND" if roblox else "CLIP"
        if hasattr(mat, "alpha_threshold"):
            mat.alpha_threshold = 0.5
        if hasattr(mat, "show_transparent_back"):
...
def _match_base_name(name):
    base = re.sub(r"\.\d+$", "", str(name or ""))
    base = re.sub(r"[._\-\s]+(low|high|cage)\d*$", "", base, flags=re.IGNORECASE)
    base = re.sub(r"\d+$", "", base)
    return normalize_match_name(base)


def apply_textures_to_objects(objects, grouped, manifest=None, strict=False, preferred=None):
    if not grouped:
        return
    materials = {}
    material_entries = []
    for texset, maps in grouped.items():
        mat_name = texset
        normal_path, _ = _map_path_and_udim(maps.get("normal"))
    ...
    preferred_names = {
        obj.name for obj in (preferred or []) if getattr(obj, "type", None) == "MESH"
    }
    assigned_slots = set()
    assigned_objects = set()
    entry_has_fuzzy = {}

    def claim(obj, slot_index, mat):
        obj.material_slots[slot_index].material = mat
        assigned_slots.add((obj.name, slot_index))
        assigned_objects.add(obj.name)

    def claim_unnamed_slot(obj, mat):
        if not obj.material_slots:
        ...
        target_idx = None
        for idx, slot in enumerate(obj.material_slots):
            if (obj.name, idx) not in assigned_slots:
                target_idx = idx
                break
        if target_idx is None:
            target_idx = 0
        claim(obj, target_idx, mat)

    for key, mat, texset in material_entries:
        if not key:
            continue
        base_key = _match_base_name(texset)
        scored = []
        for obj in mesh_targets:
            slot_hit = None
            for idx, slot in enumerate(obj.material_slots):
                if not slot.material or (obj.name, idx) in assigned_slots:
                    continue
                slot_key = normalize_match_name(slot.material.name)
                if slot_key == key or (slot_key and slot_key == base_key) or (
                    _match_base_name(slot.material.name)
                    and _match_base_name(slot.material.name) == base_key
                ):
                    slot_hit = idx
                    break
            if slot_hit is not None:
                scored.append((100, obj, slot_hit))
                continue
            lname = normalize_match_name(obj.name)
            lbase = _match_base_name(obj.name)
            bonus = 10 if obj.name in preferred_names else 0
            if (
                lname == key
                or (lbase and lbase == key)
                or lname == base_key
                or (lbase and lbase == base_key)
            ):
                scored.append((80 + bonus, obj, None))
            elif key in lname or (lname and lname in key):
                scored.append((50 + bonus, obj, None))
        if not scored:
            continue
        scored.sort(key=lambda entry: (-entry[0], entry[1].name))
        best = scored[0][0]
        if best < 80:
            entry_has_fuzzy[texset] = True
        for score, obj, slot_hit in scored:
            if best >= 80:
                if score < 80:
                    continue
            elif obj is not scored[0][1]:
                continue
            if slot_hit is not None:
                claim(obj, slot_hit, mat)
            else:
                claim_unnamed_slot(obj, mat)

    single_target = len(mesh_targets) == 1
    if single_target and not strict:
        obj = mesh_targets[0]
        has_named_slots = any(slot.material for slot in obj.material_slots)
        if not has_named_slots:
            for idx, entry in enumerate(material_entries):
                _, mat, _ = entry
                if idx < len(obj.material_slots):
                    ...
                    obj.data.materials.append(mat)
        return
    if len(material_entries) == 1 and not strict:
        _, mat, texset = material_entries[0]
        if not entry_has_fuzzy.get(texset):
            for obj in mesh_targets:
                if obj.name not in assigned_objects:
                    claim_unnamed_slot(obj, mat)
    return {"objects": len(assigned_objects), "texsets": len(material_entries)}
...
def export_fbx_objects(filepath, objects, prefs=None, strip_uvs=False, warnings=None):
    ...
    collection_states = []
    prev_selected = [
        obj for obj in bpy.context.selected_objects if object_is_valid(obj)
    ]
            ...
            for obj in export_objs:
                ...
                try:
                    ...
                    if warnings is not None:
                        warnings.append(
                            f"Skipped {orig_name}: linked object cannot be renamed"
                        )
                    continue
                dup = obj.copy()
...
@persistent
def _on_depsgraph_update_invalidate(_scene, depsgraph):
    try:
...
def collect_high_poly_candidates(context, prefs):
    ...
    low_suffixes = parse_suffixes(getattr(prefs, "low_poly_suffixes", ""))
    cage_suffixes = parse_suffixes(getattr(prefs, "cage_poly_suffixes", ""))
    if suffixes:
        for obj in context.scene.objects:
            if obj.type != "MESH":
    ...
    if get_identify_mode(scene) == "COLLECTIONS":
        ...
        if high_collection:
            for obj in collect_collection_meshes(
                ...
                if _has_any_suffix(obj.name, low_suffixes, cage_suffixes):
                    continue
                objects.append(obj)
    ...
    if get_identify_mode(scene) == "COLLECTIONS":
        ...
        if cage_collection:
            for obj in collect_collection_meshes(
                ...
                if _has_any_suffix(obj.name, low_suffixes, high_suffixes):
                    continue
                objects.append(obj)
    if selected_only and selected_names is not None:
        objects = [obj for obj in objects if obj.name in selected_names]
...
_sp_resources_dir_cache = {"checked": False, "path": None}


def _cached_sp_resources_dir(prefs=None):
    if not _sp_resources_dir_cache["checked"]:
        try:
            _sp_resources_dir_cache["path"] = sp_install_resources_dir(find_sp_exe(prefs))
        except Exception:
            _sp_resources_dir_cache["path"] = None
        _sp_resources_dir_cache["checked"] = True
    return _sp_resources_dir_cache["path"]


def scan_sp_templates(prefs=None):
    entries = []
    try:
        resources = _cached_sp_resources_dir(prefs)
        if resources:
...
def build_sp_project_settings(prefs):
    ...
    if has_custom_template and color_mode == "PAINTER_DEFAULT":
        color_key = "template"
    color_config_path = ""
    ...
    try:
        if sys.platform == "darwin":
            if sp_exe and sp_exe.lower().endswith(".app"):
                ...
                result = subprocess.run(["open", project_file])
...
def start_update_check(show_no_update=False, show_popup=True):
    ...
    global _update_check_show_no_update
    global _update_check_show_popup
    global _update_check_generation
    if _update_check_in_progress:
        _update_check_show_no_update = _update_check_show_no_update or show_no_update
        _update_check_show_popup = _update_check_show_popup or show_popup
        return
    ...
    thread = threading.Thread(target=_update_worker, args=(generation,), daemon=True)
    thread.start()
    bpy.app.timers.register(_update_poll, first_interval=0.5, persistent=True)


def _cache_size_probe(project_root, bridge_root, project_dir):
    global _cache_size_probe_thread
    global _cache_size_check_time
    global _cache_size_global
    global _cache_size_local
    global _cache_size_project_root
    try:
        global_size = folder_size_bytes(bridge_root)
        local_size = folder_size_bytes(project_dir)
        _cache_size_global = global_size
        _cache_size_local = local_size
        _cache_size_project_root = project_root
...
def get_cached_cache_sizes(context, prefs, max_age=30.0):
    ...
    if (
        ...
        if _cache_size_probe_thread is None or not _cache_size_probe_thread.is_alive():
            try:
                bridge_root = get_bridge_root(prefs)
            except Exception:
                bridge_root = None
            _cache_size_probe_thread = threading.Thread(
                target=_cache_size_probe,
                args=(project_root, bridge_root, project_dir),
                daemon=True,
            )
            _cache_size_probe_thread.start()
...
def _auto_clear_cache_tick():
    ...
    try:
        ...
        if not limit_bytes:
            return
        global_size, _local_size = get_cached_cache_sizes(context, prefs)
        if global_size <= limit_bytes:
            return
...
class GOBSPPreferences(AddonPreferences):
    ...
    sp_color_management: EnumProperty(
        ...
        items=(
            ("PAINTER_DEFAULT", "Auto (template or Legacy)", "Template decides; without a template, Legacy"),
            ("LEGACY", "Legacy (force)", "Always Legacy; a selected template is skipped"),
...
class GOB_OT_SendToSP(Operator):
    ...
    def execute(self, context):
        ...
        if (
            low_objects
            and not auto_unwrap
            and (not prefs or prefs.export_low_poly)
        ...
        try:
            ...
            self.report({"ERROR"}, "Bridge project folder is not writable")
            return {"CANCELLED"}
        ...
        if not prefs or prefs.export_low_poly:
            if not low_objects:
                ...
                return {"CANCELLED"}
            strip_uvs = auto_unwrap and not sp_project_file
            export_warnings = []
        ...
        if manifest_path:
            try:
                ...
                self.report({"ERROR"}, "Bridge metadata folder is not writable")
                return {"CANCELLED"}
        try:
        ...
        try:
            ...
            self.report({"ERROR"}, "Failed to write the bridge manifest")
            return {"CANCELLED"}
...
class GOB_OT_ImportFromSP(Operator):
    ...
    def execute(self, context):
            ...
            if candidate:
                ...
                if manifest and current_blender_file:
                    ...
                    if manifest_blender and not paths_match(manifest_blender, current_blender_file):
                        ...
                        manifest_path = None
        if manifest and current_blender_file:
            # A linked project can go stale when every send spawns a new Painter
            # project; the newest export for this Blender file is what the user
            # actually painted last, so prefer it when it is newer.
            newest_candidate = find_manifest_for_blender_file(
                roots,
                current_blender_file,
                source="substance_painter",
            )
            if newest_candidate and newest_candidate != manifest_path:
                try:
                    if newest_candidate.stat().st_mtime > manifest_path.stat().st_mtime:
                        manifest_path = newest_candidate
                        manifest = read_manifest(manifest_path)
                except OSError:
                    pass
        ...
        new_objects = []
        mesh_import_failed = False
        if mesh_path and Path(mesh_path).is_file():
            imported_objects = import_fbx(mesh_path)
            if imported_objects is None:
                mesh_import_failed = True
            else:
                new_objects = imported_objects
        ...
        if not signature_targets:
            linked_blender_manifest = None
            manifest_blender_file = get_manifest_blender_file(manifest)
            if manifest_blender_file:
                candidate = find_manifest_for_blender_file(
                    roots,
                    manifest_blender_file,
                    source="blender",
                )
                if candidate:
                    linked_blender_manifest = read_manifest(candidate)
            if linked_blender_manifest:
                signature_targets = find_signature_targets(
                    context,
                    {"mesh_signature": linked_blender_manifest.get("mesh_signature")},
                )
        ...
        if texture_paths and targets:
            applied = apply_textures_to_objects(
                ...
                manifest=manifest,
                strict=strict,
                preferred=signature_targets,
            )

        if mesh_import_failed:
            self.report(
                {"ERROR"},
                "Mesh import failed: the FBX file could not be imported",
            )
            return {"CANCELLED"}
        report_parts = []
        if new_objects:
            report_parts.append(f"{len(new_objects)} mesh(es)")
        if texture_paths:
            if applied.get("objects"):
                report_parts.append(
                    f"{applied['texsets']} texture set(s) on {applied['objects']} object(s)"
                )
            else:
                report_parts.append("no objects matched the exported textures")
        if report_parts:
            self.report({"INFO"}, "Imported: " + "; ".join(report_parts))
        else:
            self.report(
                {"WARNING"},
                "Nothing to import; export from Painter with GoB Export first",
            )
        return {"FINISHED"}
...
class GOB_OT_OpenBugReport(Operator):
    ...
    bl_label = "Report Bug"
    bl_description = "Open the GoB SP Bridge website in your browser"
...
class GOB_OT_PreviewSplit(Operator):
    ...
    def draw(self, _context):
        layout = self.layout
        method = getattr(self, "_method", "SMART")
        entries = getattr(self, "_entries", [])
        low_count = getattr(self, "_low_count", 0)
        warnings = getattr(self, "_warnings", [])
        method_label = AUTOSPLIT_METHOD_LABELS.get(method, method)
        ...
        row = layout.row()
        low_col = row.column()
        low_col.label(text=f"LOW ({low_count})")
        high_col = row.column()
        high_col.label(text=f"HIGH ({len(entries) - low_count})")
        for name, count, side, reason in entries:
...
class GOB_PT_Panel(Panel):
    ...
    def draw_send_tab(self, context, layout, prefs):
        ...
        if mode == "COLLECTIONS":
            ...
            layout.label(text="Also by name:")
            col = layout.column(align=True)
    ...
    def draw_setup_tab(self, layout, prefs):
        ...
        if template_key != "NONE" and prefs.sp_color_management == "LEGACY":
            layout.label(
                text="The template is skipped to honor the Legacy choice",
                icon="INFO",
            )
        elif template_key != "NONE" and prefs.sp_color_management not in ("PAINTER_DEFAULT", "LEGACY"):
            layout.label(
                text="The color management choice overrides the template",
                icon="INFO",
            )
...
def register():
    ...
    if _on_depsgraph_update_invalidate not in bpy.app.handlers.depsgraph_update_post:
        bpy.app.handlers.depsgraph_update_post.append(_on_depsgraph_update_invalidate)
...
def unregister():
    ...
    if _on_depsgraph_update_invalidate in bpy.app.handlers.depsgraph_update_post:
        bpy.app.handlers.depsgraph_update_post.remove(_on_depsgraph_update_invalidate)
